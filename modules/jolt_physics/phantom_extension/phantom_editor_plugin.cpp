#include "phantom_editor_plugin.h"
...
void PhantomEditorPlugin::_notification(int p_notification) {
	switch (p_notification) {
		case NOTIFICATION_ENTER_TREE: {
			...
			joint_gizmo_plugin = Ref(memnew(PhantomJointGizmoPlugin3D(editor_interface)));
			add_node_3d_gizmo_plugin(joint_gizmo_plugin);

			PopupMenu *tool_menu = memnew(PopupMenu);
			tool_menu->connect("id_pressed", callable_mp(this, &PhantomEditorPlugin::_tool_menu_pressed));
...
void PhantomEditorPlugin::_tool_menu_pressed(int32_t p_index) {
...
void PhantomEditorPlugin::_snapshots_dir_selected(const String &p_dir) {
	JoltPhysicsServer3D::get_singleton()->dump_debug_snapshots(p_dir);
}

void PhantomEditorPlugin::_dump_debug_snapshots() {
	if (debug_snapshots_dialog == nullptr) {
		...
		debug_snapshots_dialog->connect(
				"dir_selected",
				callable_mp(this, &PhantomEditorPlugin::_snapshots_dir_selected));
